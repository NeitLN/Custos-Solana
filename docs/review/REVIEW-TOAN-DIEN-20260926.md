...
| **CI chạy tay từ IP GitHub** (probe trình duyệt + live Devnet) | Lượt 1 đỏ vì probe đo cả nút của màn thực thi đang ẩn (0 px) — sửa probe chỉ đo nút được vẽ, đối chứng nút hiển thị ép 0 px vẫn đỏ. Lượt 2 đỏ vì lỗi sản phẩm thật: nút chuyển màn 161×34 px ở 375 px — sửa lên 44 px trên điện thoại. Lượt 3 (commit `e13f056`): **build · gói SDK · 3 probe trình duyệt · live Devnet · deploy đều xanh** | GitHub Actions run `36256006244` (workflow_dispatch), artifact `bang-chung-browser`, `bang-chung-live-devnet` |
