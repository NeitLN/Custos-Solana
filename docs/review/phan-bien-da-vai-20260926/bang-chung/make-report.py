...
probe='node --experimental-strip-types docs/review/phan-bien-da-vai-20260926/bang-chung/probe.mjs'
reg='node --experimental-strip-types --test --test-reporter=tap docs/review/phan-bien-da-vai-20260926/bang-chung/regression.test.mjs'
findings=[]
def add(id,title,kind,priority,role,locations,repro,current,expected,fix,test,accept,guard,deps=None):
    findings.append(dict(id=id,tieuDe=title,loai=kind,muc=priority,vai=role,viTri=locations,taiHien=repro,hienTai=current,mongDoi=expected,huongSua=fix,testHoiQuy=test,nghiemThu=accept,khongDuocLam=guard,phuThuoc=deps or []))
add('F-01','Tăng hạn mức của delegate cũ không được phát hiện','LỖI','P0','1 — auditor',
 ['packages/core/src/l2/rules.ts:131','packages/core/src/l1/fetch.ts:375','packages/core/src/facts.ts:14','packages/core/src/diff.ts:209','packages/ai/src/nhanDien.ts:82'],
 f'Chạy `{probe}`; xem dòng `F-01 L1-to-L3` trong `bang-chung/probe.log`. Probe dựng SPL account bằng AccountLayout và giao dịch Approve chưa ký, RPC stub có trạng thái trước/sau. Delegate không đổi; hạn mức từ 1 DEMO lên u64::MAX; mint đã thu hồi mint/freeze authority. Ví được bảo vệ là ví cố định. Không gửi giao dịch.',
 '`level=safe`, `reasonCodes=[]`, coverage 1/1; diff chỉ còn phí; L3 nói “Không tìm thấy dấu hiệu nào trong danh sách chúng tôi kiểm tra được.” L2 bỏ qua ngay khi địa chỉ delegate không đổi. L1 chỉ giữ hạn mức SAU nên tầng sau không phân biệt tăng/giảm/giữ nguyên. Đây là lỗi toàn tuyến, không chỉ một mock trực tiếp của L2.',
 'Nhận diện việc mở rộng quyền rút dù người được cấp quyền không đổi. Approve ghi lại delegate và allowance; tài liệu chính thức: [Solana — Approve Delegate](https://solana.com/docs/tokens/basics/approve-delegate). Ca tăng 1 → MAX phải kích hoạt R03 và hiện rõ quyền mới; không nói token đã chuyển ngay.',
 'A bổ sung hạn mức trước vào Facts nội bộ, lấy từ account đã đọc. L2 phân biệt đổi delegate và tăng allowance. A/C cập nhật diff + nhận diện để cùng phản ánh thay đổi. Ít rủi ro hơn việc xoá đơn thuần điều kiện “cùng delegate”: xoá điều kiện sẽ có thể cáo buộc cả giao dịch không thay quyền. Với fixture cũ thiếu hạn mức trước, giữ trạng thái chưa biết và chọn xử lý thận trọng có tài liệu; không tự coi trước=0.',
 'Đã có test F-01 ĐỎ trong regression.test.mjs. Thêm: cùng delegate tăng 1→MAX; cấp mới MAX; cùng delegate giữ nguyên; giảm hạn mức; tăng nhưng vẫn trong ngưỡng hợp lệ; chuyển token không đổi quyền. Kiểm cả mã R03, dòng allowance và lời L3, không chỉ màu.',
 f'`{reg}`: riêng F-01 phải xanh sau sửa; bộ `node --test packages/core/test/l2.test.ts packages/core/test/inspect.test.ts packages/ai/test/nhanDien.test.ts` vẫn qua. Cuối đợt cả sáu test mới phải xanh.',
 'Chạm L1/L2/diff/L3, owner A+C. Không biến mọi Approve thành Đỏ; không cho L3 sửa level; không đổi các trường bắt buộc của InspectResult.')
add('F-08','Không đọc được mint vẫn trả safe và tự gán decimals=0','LỖI','P0','1 — auditor',
 ['packages/core/src/l1/fetch.ts:449','packages/core/src/l1/fetch.ts:454','packages/core/src/diff.ts:159','packages/core/src/l2/rules.ts:471'],
 f'`{probe}`, dòng `F-08 missing mint`. Cùng giao dịch chuyển 1 token, cùng account buffers và mô phỏng; chỉ đổi lượt đọc mint từ buffer hợp lệ sang null. Mint đối chứng có freeze authority; không gửi giao dịch.',
 'Đủ mint: warning, FREEZE_AUTHORITY_CON_HIEU_LUC, số dư 500,0→499,0. Thiếu mint: safe, không mã lý do, coverage 1/1, số dư 500.000.000→499.000.000, soLieu.decimals=0. Đây là hai biểu hiện của cùng nguyên nhân: dữ liệu nền bắt buộc bị xử lý như metadata tên token tuỳ chọn.',
 'Thiếu mint liên quan phải được công khai là thiếu dữ liệu phân tích, không suy “không có quyền đóng băng/extension”. Không biến đơn vị thô thành số lượng token bằng cách đoán decimals. Quyết định fail-safe #4 áp dụng; ký hiệu token và tuổi ví vẫn là dữ liệu làm giàu tuỳ chọn.',
 'Theo dõi mint liên quan chưa đọc/parse được trong Facts, L2 trả warning do thiếu dữ kiện. Giữ coverage instruction đúng nghĩa, thêm thông báo thiếu dữ liệu thay vì giả vờ không decode được lệnh. Bảng/L3 không format số token khi chưa biết decimals: hiện đơn vị cơ sở có nhãn rõ hoặc bỏ giá trị quy đổi; không xuất soLieu.decimals=0 giả. Có thể retry đọc mint một lần trong ngân sách hiện có, nhưng hết hạn vẫn phải suy giảm đúng.',
 'Test F-08 hiện ĐỎ. Bổ sung mint trả null, throw, buffer sai và thiếu Token-2022 extension; mint decimals=0 THẬT phải vẫn hợp lệ; chỉ mất symbol/PDA hoặc tuổi ví không tự làm warning. Đối chiếu hiển thị và dữ liệu có cấu trúc.',
 f'`{reg}`: F-08 xanh. Thêm test tích hợp L1→L2 và diff vào suite core; ca đủ mint vẫn giữ cảnh báo thuộc tính hiện có, ca thiếu mint không safe và không giả decimals.',
 'Chạm L1/L2/diff/L3, A+C. Không coi freeze/PD/hook là độc hại chỉ vì tồn tại; không áp fail-safe cho mọi lỗi metadata tên token; không phá InspectResult.')
add('F-07','Mảng accounts bị cắt được đọc thành tài khoản về 0','LỖI','P1','1 — auditor',
 ['packages/core/src/l1/fetch.ts:270','packages/core/src/l1/fetch.ts:322'],
 f'`{probe}`, dòng `F-07 truncated simulation accounts`: trước ví có 1 SOL, tx chuyển 1 lamport, RPC stub trả err:null và accounts:[] dù đã yêu cầu account.',
 'Diff tuyên bố 1,0→0,0 SOL, reason SOL_ROI_VI, coverage 1/1. Array.isArray đủ để coDuLieuAccount=true; ô KHÔNG CÓ được đổi thành null rồi coi là trạng thái tài khoản không còn. Đã phân biệt accounts:null, nhưng chưa phân biệt mảng thiếu phần tử với phần tử null hợp lệ.',
 'Dữ liệu thiếu phải thành “chưa đo”, không thành mất tiền. Đây không phải khẳng định RPC công cộng hiện trả phản hồi lỗi này: lỗi xử lý phản hồi đã tái hiện bằng transport stub tại ranh giới tin cậy.',
 'Kiểm độ dài và sự hiện diện từng index trước khi đưa vào afterByIndex. Đánh dấu các địa chỉ thiếu ở accountKhongDoDuoc; chỉ null HIỆN DIỆN ở đúng vị trí mới có nghĩa account không tồn tại. Có thể từ chối toàn bộ phản hồi sai hình dạng; xử lý từng vị trí giữ được nhiều dữ kiện hơn nhưng cần test kỹ.',
 'Test F-07 hiện ĐỎ. Thêm [], mảng ngắn, sparse array và account data malformed. Đối chứng mảng đủ độ dài có một null do CloseAccount phải giữ hành vi hợp lệ, không biến thành số liệu thiếu toàn bộ.',
 f'`{reg}`: F-07 xanh, có TRANG_THAI_DO_KHUYET và không dựng dòng SOL 1→0 từ ô không tồn tại; `node --test packages/core/test/failsafe.test.ts packages/core/test/dong-tai-khoan.test.ts` qua.',
 'Chạm L1, A. Không gộp “missing” với “đã đóng”; không nâng thiếu dữ liệu thành cáo buộc drainer; không nới fail-safe.')
add('F-04','Lời AI vẫn tự kết luận nguy hiểm và khuyên ký','LỖI','P0','6 — AI/LLM',
 ['packages/ai/src/moHinh.ts:93','packages/ai/src/moHinh.ts:234','packages/ai/src/moHinh.ts:429'],
 f'`{probe}`, ba dòng `F-04`. Callback GoiMoHinh giả lập trả JSON đúng schema trên Facts cấp quyền hợp lệ. Đây là phép thử validator, KHÔNG phải đo mô hình thật hay chứng minh token metadata đã prompt-inject thành công.',
 'Cả ba câu được nhận nguyên văn: “Giao dịch này nguy hiểm.”; “Bạn có thể ký mà không phải kiểm tra thêm.”; “Tài sản của bạn được bảo vệ tuyệt đối.” Bộ chặn chủ yếu bắt một danh sách cụm trấn an; những câu không có số/địa chỉ cũng qua các neo khác. Enum level không bị sửa, nhưng câu người dùng đọc vi phạm quyết định #1.',
 'L3 chỉ mô tả hậu quả có căn cứ hoặc đề nghị kiểm tra thủ công; không tự xác nhận an toàn, không tự kết luận nguy hiểm, không bảo người dùng bỏ kiểm tra.',
 'Trước mắt thêm nhóm kiểm câu phán quyết/xúi ký và ca phủ định/phủ định kép; trả về câu tất định khi vi phạm. Hướng bền hơn: để mô hình chọn/diễn đạt các mệnh đề hậu quả đã định danh, UI dựng phần cảnh báo quan trọng từ dữ kiện. Không coi việc thêm ba regex là chứng minh đã loại mọi biến thể ngôn ngữ. Giữ một bộ đối kháng ngoài các mẫu dùng để sửa.',
 'Test F-04 hiện ĐỎ trên ba câu. Thêm paraphrase, Unicode/zero-width, phủ định, câu đặt số đúng vào quan hệ sai; đối chứng lời mô tả “tài khoản đổi chủ” có bằng chứng vẫn được nhận. Assert L2 nguyên vẹn và fallback còn đủ hậu quả.',
 f'`{reg}`: F-04 xanh; `node --test packages/ai/test/moHinh.test.ts packages/ai/test/boiThoiHanC05.test.ts` qua. Báo riêng số câu bị chặn sai/cho lọt trên tập giữ lại; không tự đặt tỷ lệ chưa đo.',
 'Chạm L3, C. Không cho AI điều chỉnh level/reasonCodes; không bỏ cảnh báo tất định vì AI lỗi; không nhúng khoá API vào browser.')
add('F-02','Interpreter nhận tham chiếu có thể sửa dữ kiện L2 trả về','LỖI','P1','5 — senior maintainer',
 ['packages/core/src/inspect.ts:158','packages/core/src/inspect.ts:266','packages/core/src/inspect.ts:267'],
 f'`{probe}`, dòng `F-02`. Callback interpret thực hiện reasonCodes.splice(0) và sửa facts.coverage.analyzed=123 rồi trả về bình thường.',
 'Kết quả vẫn warning nhưng reasonCodes thành [], coverage 123/1. Facts và mảng l2.reasonCodes được đưa cho callback bằng tham chiếu; locL3 chỉ lọc GIÁ TRỊ TRẢ VỀ. Không chứng minh LLM từ xa chạy được JavaScript này: điều kiện là adapter do consumer cung cấp có bug hoặc không đáng tin. Diff đã dựng trước nên không bị đổi trực tiếp trong probe.',
 'Quyết định #1 và lời cam kết trong inspect.ts rằng L3 hỏng không làm mất reasonCodes phải đúng cả ở runtime. Plugin diễn giải không được làm hỏng dữ kiện trả về hoặc chỉ mục bằng chứng tạo sau await.',
 'Truyền bản sao sâu của Facts/options và bản sao reasonCodes vào interpreter, giữ snapshot gốc cho result/diagnostic. Có thể deep-freeze bản sao để bắt adapter sửa nhầm; ưu tiên cách không freeze object do consumer sở hữu. Readonly TypeScript bổ sung tài liệu nhưng không thay kiểm runtime. Kiểm cả callback giữ tham chiếu và sửa sau timeout.',
 'Test F-02 hiện ĐỎ. Thêm mutation trước resolve, trước throw, sau resolve, sau timeout; canh reasonCodes, coverage, diagnostic và input options không đổi. Ca interpreter đọc bình thường không được suy giảm.',
 f'`{reg}`: F-02 xanh; `node --test packages/core/test/inspect.test.ts packages/core/test/chanDoanX01.test.ts` qua; kết quả warning phải còn MO_PHONG_HONG và coverage không vượt total.',
 'Chạm biên SDK/L3, A+C. Không chạy lại RPC để lấy “dữ kiện sạch”; không cho interpreter sửa level; không thay giao kèo công khai.')
add('F-03','Ví dụ tích hợp cho ký dù có yêu cầu kiểm tra thủ công','LỖI','P1','3 — kỹ sư tích hợp',
 ['vi-du-tich-hop/src/tich-hop.js:75','vi-du-tich-hop/src/tich-hop.js:83','packages/core/README.md:92','apps/demo-wallet/src/WalletExecution.tsx:118','apps/demo-wallet/src/live/session.ts:317'],
 f'`{probe}`, dòng `F-03`: inject InspectResult safe + coverage đầy đủ + aiAdvisory=review_required vào kiemTruocKhiKy.',
 'Helper trả cho:"ky", lyDo:"khong_van_de". Ví dụ “đầy đủ” trong README cũng chỉ xét level/coverage, trái với đoạn ngắn ngay đầu README xét cả aiAdvisory. WalletExecution/execute cũng chỉ dùng level để phân biệt approve/override; UI vẫn hiện advisory nên không gọi đó là mất hiển thị, nhưng nhật ký và câu đồng ý không thể hiện người dùng đã bỏ qua advisory.',
 'Advisory phải được consumer xử lý như yêu cầu người dùng xem lại; không cần đổi level sang warning. Việc tiếp tục ký phải là quyết định rõ ràng theo policy của consumer, không được gọi “không vấn đề” rồi cho đi thẳng.',
 'Thêm nhánh advisory sau các mức L2 mạnh hơn và trước trả cho:"ky"; chọn hoi với lý do có tài liệu. Đồng bộ snippet/test executable. Rà đường xác nhận của ví mẫu để advisory có bước xác nhận đọc hiểu và ghi nhật ký tương ứng, vẫn cho phép override có chủ ý.',
 'Test F-03 hiện ĐỎ. Ma trận safe+null, safe+advisory, warning, danger, coverage thiếu và inspect throw/timeout. Với lời khai lệch mà L2 safe, helper phải hoi; lời khai khớp không được xoá advisory đã có.',
 f'`{reg}`: F-03 xanh, safe+advisory trả hoi; thêm test consumer thật trong suite thay vì chỉ tìm chuỗi README. Chạy lại các test ví dụ tích hợp tất định sau khi rà script bảo đảm chỉ ghi trong phạm vi được phép.',
 'Chạm consumer policy/README/ví mẫu, A+B+C. Không nâng level từ AI; không hạ danger thành hoi; không biến ngoại lệ RPC thành được ký.')
add('F-05','URL guest vẫn thay ví mặc định đã khoá','LỖI','P1','4 — người dùng Việt Nam',
 ['apps/demo-wallet/src/WalletExecution.tsx:23','apps/demo-wallet/src/WalletExecution.tsx:28','apps/demo-wallet/src/WalletExecution.tsx:35','apps/demo-wallet/src/WalletExecution.tsx:191','apps/demo-wallet/test/viDemoCoDinh.test.ts:62'],
 'Dựng và chạy browser theo mục 4. Mở `http://127.0.0.1:5197/?guest=1&wallet=724Me67n3Rfc4G6FisqPFi1ukt5qEfcFQQ3t1vBWHw3G`. Xem guest-public-address.png và browser.json. Chỉ truyền địa chỉ CÔNG KHAI cũ; không sinh khoá, không upload, không ký. Code còn cho phép ?guest=1 không địa chỉ tự gọi Keypair.generate; nhánh này chỉ đọc code, không chạy browser.',
 'UI đổi sang địa chỉ 724Me… và ghi “Ví khách riêng”. Landing ví cố định có link “Tự thử bằng ví khách riêng”. Guard viDemoCoDinh kiểm vi.ts nhưng không kiểm WalletExecution mới. Sáu test ví cố định vẫn qua trong suite 162 bài.',
 'Mọi entry demo phải dùng AqX3FmDzuU1a9FAPpmo9m52ckQFBeExcGhs8qbPEBCLZ như quyết định #8 trong AGENTS.md:100. Không đẩy người vừa nạp faucet vào một ví khác.',
 'Bỏ khả năng URL/link thay public wallet trong đường UI sản phẩm; query cũ được bỏ qua hoặc thông báo đã về ví chuẩn. Test seam constructor có thể giữ cho unit test, nhưng UI không truyền override từ URL. Không xoá dữ liệu phiên cũ hoặc key file của người dùng.',
 'Browser regression: không query, guest=1, guest=1&wallet=địa_chỉ_khác, địa chỉ lỗi và reload đều hiện ví chuẩn; không có nút tạo/tải ví khách. Kiểm DOM địa chỉ đầy đủ thay vì chỉ nhãn rút gọn. Ca hiện tại query địa chỉ cũ sẽ ĐỎ.',
 'Chạy lại build-review.mjs và browser-review.py. Artifact guest-public-address phải vẫn hiện ví AqX3…EBCLZ; không có Ví khách riêng/Tự thử bằng ví khách riêng. Test viDemoCoDinh vẫn qua. Không dùng khoá thật để test.',
 'Chạm ví mẫu, B. File UI/session là mã CHƯA COMMIT. Không tạo ví demo mới, không đổi faucet đích, không đụng .devnet hoặc .env.')
add('F-06','So quyền đọc ở slot sau dễ bị hiểu thành hậu quả của giao dịch cũ','RỦI RO','P2','4 — UX, phối hợp auditor',
 ['apps/demo-wallet/src/live/receipt.ts:35','apps/demo-wallet/src/live/receipt.ts:44','apps/demo-wallet/src/live/session.ts:370','apps/demo-wallet/src/live/Receipt.tsx:26'],
 f'`{probe}`, dòng `F-06`: giao dịch ở slot 100, quyền đọc lại slot 900 khác dự báo. compareReceipt trả authority:mismatch. Hàm đã tái hiện; chưa thực hiện hai giao dịch thật để chứng minh chuỗi sự kiện ngoài mạng.',
 'So sánh chấp nhận mọi ownerSlot >= transaction slot. UI có ghi slot và footnote trạng thái có thể thay đổi tiếp — điểm giảm rủi ro đã có. Tuy nhiên cột “Đối chiếu”/“Có chênh lệch” dưới tiêu đề “Dự báo gặp thực tế” vẫn có thể bị đọc thành dự báo sai cho chính giao dịch cũ. Delegate bị sử dụng hoặc quyền bị đổi ở giao dịch sau là điều kiện cụ thể.',
 'Tách đối chiếu metadata của đúng giao dịch với đối chiếu snapshot hiện tại. Snapshot sau chỉ chứng minh trạng thái lúc đọc, không chứng minh quyền ngay sau giao dịch; cùng slot cũng không đủ chứng minh không có giao dịch khác trong slot.',
 'Ít rủi ro: giữ số dư theo transaction metadata; các quyền hiện tại có nhãn “Trạng thái hiện tại tại slot …; chưa quy được cho riêng giao dịch này”. Không tính chúng vào pass/fail dự báo lịch sử. Phương án sâu hơn cần nguồn lịch sử có thể quy thuộc thay đổi cho đúng transaction, không chỉ minContextSlot.',
 'Nếu chọn hướng tách đối chiếu: test slot100 + read900 khác quyền phải có historical authority=unknown, vẫn hiện current snapshot; test khớp giá trị cũng không được biến thành bằng chứng lịch sử chắc chắn. Test kỳ vọng unknown sẽ ĐỎ ở mã hiện tại. Đây là thay đổi semantics nội bộ cần B thống nhất, không phải khẳng định RPC cung cấp lịch sử.',
 'Thêm test vào apps/demo-wallet/test/liveDemo.test.ts rồi chạy `node --test apps/demo-wallet/test/liveDemo.test.ts`; fixture có một thay đổi quyền xen giữa không bị ghi là Custos dự báo sai; số dư theo metadata vẫn đối chiếu được.',
 'Chạm receipt/UI, B; mã CHƯA COMMIT. Không thay owner hiện tại bằng owner dự báo; không giả lịch sử; không yêu cầu gửi giao dịch trong vòng kiểm read-only này.')

header='''# Custos — phản biện sản phẩm qua sáu vai, bàn giao Claude
...
'''
parts=[header]
labels=[('taiHien','Tái hiện'),('hienTai','Hiện tại'),('mongDoi','Mong đợi'),('huongSua','Hướng sửa'),('testHoiQuy','Test hồi quy'),('nghiemThu','Nghiệm thu'),('khongDuocLam','Không được làm')]
for f in findings:
    parts.append(f"### {f['id']} · {f['tieuDe']} — {f['loai']} · {f['muc']}\n\n- **Vai phát hiện:** {f['vai']}\n- **Vị trí:** "+' · '.join('`'+x+'`' for x in f['viTri'])+'\n')
    for field,label in labels: parts.append(f"- **{label}:** {f[field]}\n")
    parts.append('- **Phụ thuộc:** '+(', '.join(f['phuThuoc']) or 'Không có finding bắt buộc phải sửa trước.')+'\n\n')
parts.append('''## 3. Kỹ thuật né của vai 2
...
''')
(root/'BAO-CAO.md').write_text(''.join(parts),encoding='utf-8')
fields=['id','tieuDe','loai','muc','vai','viTri','taiHien','nghiemThu','phuThuoc']
(root/'findings.json').write_text(json.dumps([{k:f[k] for k in fields} for f in findings],ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
print('Wrote report and',len(findings),'findings.')
