...
- Bằng chứng tích hợp đo tại: `cf53ebc`, cây làm việc sạch — lượt gần nhất PASS
...
|---|---|---|
| Test tự động | **462** pass · 0 fail | `npm run check` |
...
| — dải đo | **7,2–15,9 giây**, trung vị 10 lượt trên 10 bản dựng | `data/tich-hop/ket-qua.json` → `lichSuPass` |
| — dòng mã tích hợp | **30** | `vi-du-tich-hop/src/tich-hop.js` |
| — một lượt `inspect()` | **680 ms** | cùng file |
...
|---|---|---|---|
| 1 | Lượt tích hợp gần nhất pass hay fail? | **PASS** tại `cf53ebc` | `ket-qua.json` → `lastAttempt` |
...
- **462 test chứng minh code có kỷ luật, KHÔNG chứng minh độ chính xác ngoài đời.**
