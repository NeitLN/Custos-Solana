...
- Bằng chứng tích hợp đo tại: `3fd51af`, cây làm việc sạch — lượt gần nhất PASS
...
|---|---|---|
| Test tự động | **791** pass · 0 fail | `npm run check` |
...
| Tích hợp từ ngoài monorepo | **8,3 giây** tới kết quả đầu | `npm run thu-tich-hop:devnet` |
...
| — một lượt `inspect()` | **632 ms** | cùng file |
...
|---|---|---|---|
| 1 | Lượt tích hợp gần nhất pass hay fail? | **PASS** tại `3fd51af` | `ket-qua.json` → `lastAttempt` |
...
- **791 test chứng minh code có kỷ luật, KHÔNG chứng minh độ chính xác ngoài đời.**
