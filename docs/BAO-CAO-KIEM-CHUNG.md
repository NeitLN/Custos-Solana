...
- Bằng chứng tích hợp đo tại: `08f50a9`, cây làm việc sạch — lượt gần nhất PASS
...
|---|---|---|
| Test tự động | **749** pass · 0 fail | `npm run check` |
...
| Tích hợp từ ngoài monorepo | **9,1 giây** tới kết quả đầu | `npm run thu-tich-hop:devnet` |
| — dải đo | **7,5–15,4 giây**, trung vị 10 lượt trên 10 bản dựng | `data/tich-hop/ket-qua.json` → `lichSuPass` |
| — dòng mã tích hợp | **30** | `vi-du-tich-hop/src/tich-hop.js` |
| — một lượt `inspect()` | **633 ms** | cùng file |
...
|---|---|---|---|
| 1 | Lượt tích hợp gần nhất pass hay fail? | **PASS** tại `08f50a9` | `ket-qua.json` → `lastAttempt` |
...
- **749 test chứng minh code có kỷ luật, KHÔNG chứng minh độ chính xác ngoài đời.**
