...
- Bằng chứng tích hợp đo tại: `a715ee1`, cây làm việc sạch — lượt gần nhất PASS
...
|---|---|---|
| Test tự động | **429** pass · 0 fail | `npm run check` |
...
| — dải đo | **7–15,8 giây**, trung vị 10 lượt trên 8 bản dựng | `data/tich-hop/ket-qua.json` → `lichSuPass` |
| — dòng mã tích hợp | **30** | `vi-du-tich-hop/src/tich-hop.js` |
| — một lượt `inspect()` | **625 ms** | cùng file |
...
|---|---|---|---|
| 1 | Lượt tích hợp gần nhất pass hay fail? | **PASS** tại `a715ee1` | `ket-qua.json` → `lastAttempt` |
...
- **429 test chứng minh code có kỷ luật, KHÔNG chứng minh độ chính xác ngoài đời.**
