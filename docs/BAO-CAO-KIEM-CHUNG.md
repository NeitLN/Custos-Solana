...
- Bằng chứng tích hợp đo tại: `753da81`, cây làm việc sạch — lượt gần nhất PASS
...
|---|---|---|
| Test tự động | **403** pass · 0 fail | `npm run check` |
...
| — dải đo | **6,9–11,8 giây**, trung vị 10 lượt trên 7 bản dựng | `data/tich-hop/ket-qua.json` → `lichSuPass` |
| — dòng mã tích hợp | **30** | `vi-du-tich-hop/src/tich-hop.js` |
| — một lượt `inspect()` | **615 ms** | cùng file |
...
|---|---|---|---|
| 1 | Lượt tích hợp gần nhất pass hay fail? | **PASS** tại `753da81` | `ket-qua.json` → `lastAttempt` |
...
- **403 test chứng minh code có kỷ luật, KHÔNG chứng minh độ chính xác ngoài đời.**
