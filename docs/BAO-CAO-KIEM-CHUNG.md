...
- Bằng chứng tích hợp đo tại: `37b9d88`, cây làm việc sạch — lượt gần nhất PASS
...
|---|---|---|
| Test tự động | **473** pass · 0 fail | `npm run check` |
...
| Tích hợp từ ngoài monorepo | **11,8 giây** tới kết quả đầu | `npm run thu-tich-hop:devnet` |
| — dải đo | **7,8–23,5 giây**, trung vị 10 lượt trên 10 bản dựng | `data/tich-hop/ket-qua.json` → `lichSuPass` |
| — dòng mã tích hợp | **30** | `vi-du-tich-hop/src/tich-hop.js` |
| — một lượt `inspect()` | **672 ms** | cùng file |
...
|---|---|---|---|
| 1 | Lượt tích hợp gần nhất pass hay fail? | **PASS** tại `37b9d88` | `ket-qua.json` → `lastAttempt` |
...
- **473 test chứng minh code có kỷ luật, KHÔNG chứng minh độ chính xác ngoài đời.**
