...
- Bằng chứng tích hợp đo tại: `50b8728`, cây làm việc sạch — lượt gần nhất PASS
...
|---|---|---|
| Test tự động | **783** pass · 0 fail | `npm run check` |
...
| Tích hợp từ ngoài monorepo | **8,6 giây** tới kết quả đầu | `npm run thu-tich-hop:devnet` |
| — dải đo | **6,9–13,9 giây**, trung vị 10 lượt trên 10 bản dựng | `data/tich-hop/ket-qua.json` → `lichSuPass` |
| — dòng mã tích hợp | **30** | `vi-du-tich-hop/src/tich-hop.js` |
| — một lượt `inspect()` | **631 ms** | cùng file |
...
|---|---|---|---|
| 1 | Lượt tích hợp gần nhất pass hay fail? | **PASS** tại `50b8728` | `ket-qua.json` → `lastAttempt` |
...
- **783 test chứng minh code có kỷ luật, KHÔNG chứng minh độ chính xác ngoài đời.**
