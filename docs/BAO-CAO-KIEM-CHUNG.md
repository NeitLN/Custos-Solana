...
- Bằng chứng tích hợp đo tại: `905566d`, cây làm việc sạch — lượt gần nhất PASS
...
| Tích hợp từ ngoài monorepo | **7,2 giây** tới kết quả đầu | `npm run thu-tich-hop:devnet` |
| — dải đo | **7–15,8 giây**, trung vị 10 lượt trên 9 bản dựng | `data/tich-hop/ket-qua.json` → `lichSuPass` |
| — dòng mã tích hợp | **30** | `vi-du-tich-hop/src/tich-hop.js` |
| — một lượt `inspect()` | **645 ms** | cùng file |
...
|---|---|---|---|
| 1 | Lượt tích hợp gần nhất pass hay fail? | **PASS** tại `905566d` | `ket-qua.json` → `lastAttempt` |
