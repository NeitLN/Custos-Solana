...
|---|---|
| SDK cài được từ ngoài repo chưa? | **Rồi** — 9,4 giây từ `npm install` tới kết quả đầu tiên, 30 dòng mã tích hợp |
...
|---|---|
| Cài đặt → kết quả đầu tiên | **9,4 giây** — trung vị 10 lượt trên 10 bản dựng, dải 7–15,9 |
| Dòng mã tích hợp | **30** |
| Một lượt kiểm tra | **671 ms** — trung vị 10 lượt trên 10 bản dựng |
...
| Test | **440**, chạy trong `npm run check` |
...
|---|---|---|
| **440 test** tự động | code giữ đúng bất biến đã khoá | độ chính xác ngoài đời thật |
...
npx npm@11.6.2 run check # typecheck + 440 test
...
Measured, not estimated: **440 tests**, **38 labelled samples**, **14 rules**, average
