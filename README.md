...
|---|---|
| SDK cài được từ ngoài repo chưa? | **Rồi** — 11,8 giây từ `npm install` tới kết quả đầu tiên, 30 dòng mã tích hợp |
...
| Đo trên Devnet, 09/09/2026 — lượt pass gần nhất | |
|---|---|
| Cài đặt → kết quả đầu tiên | **11,8 giây** — trung vị 10 lượt trên 10 bản dựng, dải 7,8–23,5 |
| Dòng mã tích hợp | **30** |
| Một lượt kiểm tra | **677 ms** — trung vị 10 lượt trên 10 bản dựng |
...
| Test | **467**, chạy trong `npm run check` |
...
|---|---|---|
| **467 test** tự động | code giữ đúng bất biến đã khoá | độ chính xác ngoài đời thật |
...
npx npm@11.6.2 run check # typecheck + 467 test
...
Measured, not estimated: **467 tests**, **38 labelled samples**, **14 rules**, average
