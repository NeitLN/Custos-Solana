...
|---|---|
| SDK cài được từ ngoài repo chưa? | **Rồi** — 9,9 giây từ `npm install` tới kết quả đầu tiên, 29 dòng mã tích hợp |
...
|---|---|
| Cài đặt → kết quả đầu tiên | **9,9 giây** |
| Dòng mã tích hợp | **29** |
| Một lượt kiểm tra | **595 ms** — trung vị 5 lượt |
...
| Test | **377**, chạy trong `npm run check` |
...
|---|---|---|
| **377 test** tự động | code giữ đúng bất biến đã khoá | độ chính xác ngoài đời thật |
...
npx npm@11.6.2 run check # typecheck + 377 test
...
Measured, not estimated: **377 tests**, **33 labelled samples**, **14 rules**, average
