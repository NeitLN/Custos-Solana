...
|---|---|
| **30 %** độ khó và chiều sâu | **14** luật L2, mỗi luật có ca dương **và** ca đối chứng · **843** test offline · CPI/inner instruction, ALT, legacy, nhiều signer · Token-2022 **2/26 extension** đọc được · [ma trận hành vi](docs/bao-mat/MA-TRAN-HANH-VI.md) 19 họ ca · [threat model](docs/bao-mat/THREAT-MODEL.md) 8 rủi ro |
...
| **25 %** Solana stack · hiệu năng | **7** chương trình đọc hiểu qua IDL **công bố trên chuỗi**, 245 mã lệnh · coverage **82 %** · **6,5** lượt RPC/lượt kiểm · `inspect()` **596 ms** · [ngân sách RPC](docs/NGAN-SACH-RPC.md) |
...
|---|---|
| SDK cài được từ ngoài repo chưa? | **Rồi** — 8,9 giây từ `npm install` tới kết quả đầu tiên, 30 dòng mã tích hợp |
...
|---|---|
| Cài đặt → kết quả đầu tiên | **8,9 giây** — trung vị 10 lượt trên 10 bản dựng, dải 7,1–11,8 |
| Dòng mã tích hợp | **30** |
| Một lượt kiểm tra | **596 ms** — trung vị 10 lượt trên 10 bản dựng |
...
| Test | **843**, chạy trong `npm run check` |
...
|---|---|---|
| **843 test** tự động | code giữ đúng bất biến đã khoá | độ chính xác ngoài đời thật |
...
npx npm@11.6.2 run check # typecheck + 843 test
...
Measured, not estimated: **843 tests**, **38 labelled samples**, **14 rules**, average
